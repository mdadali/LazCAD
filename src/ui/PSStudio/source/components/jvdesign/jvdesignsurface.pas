...
type
  TJvDesignControlEvent = procedure(Sender: TObject; AControl: TControl) of object; //maurog
  ...
  TJvDesignGetAddClassEvent = procedure(Sender: TObject; var ioClass: string) of object;
{
  ...
  private
    FOnControlDblClick: TJvDesignControlEvent; //maurog
    FActive: Boolean;
    FAddClass: string;
    FContainer: TWinControl;
...
//    FOnOwnerDrawGrid: TJvDesignOwnerDrawGridEvent;
  ...
  published
    property OnControlDblClick: TJvDesignControlEvent  //maurog
      read FOnControlDblClick write FOnControlDblClick;
...
begin
  ...
  //DoubleClick behandeln
  if AMessage.Msg = LM_LBUTTONDBLCLK then
  begin
    pt := MousePoint;
    ...
    if Assigned(Container)  then
      surface := TJvDesignPanel(Container).Surface;

    if Assigned(surface) then
    begin
      ctrl := surface.FindControl(pt.X, pt.Y);

      if Assigned(ctrl) and Assigned(surface.FOnControlDblClick) then
        surface.FOnControlDblClick(surface, ctrl);
    end;

    Result := True;
    Exit; //wichtig: nicht weiter durchreichen
  end;
  ...
  case AMessage.Msg of
    ...
    LM_KEYDOWN..LM_KEYUP, LM_PAINT, LM_ERASEBKGND,
    LM_WINDOWPOSCHANGED, CN_KEYDOWN..CN_KEYUP:
      Result := FOnDesignMessage(ASender, AMessage, Point(0, 0));
