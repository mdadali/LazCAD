...
uses SysUtils, Classes, LCLType, Graphics, Controls, ClipBrd, ComCtrls,
  CS4BaseTypes, LMessages, LCLIntf, FPCanvas;

procedure ReadPenFromStream(APen: TPen; AStream: TStream);
procedure WritePenToStream(APen:  TPen; AStream: TStream);

function  MirrorY2D: TTransf2D;
function  MirrorX2D: TTransf2D;

procedure SwapPoints2D(var P0, P1: TPoint2D);
function  AngleFromPoints2D(P0, P1: TPoint2D): TRealType;
procedure DrawArrows2D(const VT: TTransf2D; const Cnv: TDecorativeCanvas; const ClipRect2D: TRect2D; const DrawMode: Integer; P0, P1: TPoint2D; AArrowLength,  AArrowAngle: TRealType; AArrowStyle: word);
function  CrossingPoint2D(P0, P1, P2, P3: TPoint2D; var P4:TPoint2D ): boolean;
...
procedure MakePolarPoint2D(LastPt: TPoint2D; var CurrSnappedPt: TPoint2D; AAngle: TRealType);
procedure Make45_2D(LastPt: TPoint2D; var CurrSnappedPt: TPoint2D);
...
function  ShiftPointScale(const P: TPoint2D; const V: TVector2D; const S: TRealType): TPoint2D;
...
{: This type define the general type for source block names.
  ...
  TSourceBlockName = array[0..128] of Char;
...
TPosition2DInsp = class(TPersistent)
  ...
  procedure SetLeft(ALeft: TRealType);
  function  GetBottom: TRealType;
  procedure SetBottom(ABottom: TRealType);
  function  GetRight: TRealType;
  procedure SetRight(ARight: TRealType);
public
  constructor create(AOwner: TGraphicObject);
  destructor  destroy; override;
published
...
TGrObjectInsp = class(TPersistent)
    ...
    public
      ...
      destructor  destroy; override;
    published
      property ObjectClass: string      read GetClassName;
      property ObjectID:    LongInt     read GetID;
      property LayerIndex:  word        read GetLayerIDX  write SetLayerIDX;
...
{: This property contains the layer on which the object lies.
   ...
   By Dafault this property is set to <B=0>.
    ...
    property LayerIndex: Byte read fLayerIndex write fLayerIndex;
    property ObjectClass: string read GetClassName;
    property ObjectID: LongInt read fID;
    //added
    property LayerName: TLayerName  read GetLayerName write SetLayerName;
...
{: This property may be used to store some user's defined data
   ...
   This is only a simple and not optimized way to handle these problems.
    ...
    published
      property GraphicObject: TGrObjectInsp read  fGrObjectInsp write  fGrObjectInsp;
...
{: This type defines the layer used by the library.
  ...
  public
    procedure Assign(ALayer: TLayer);
...
{: This class defines a set of 256 layers (from 0 to 255).
  ...
  public
    procedure Assign(ALayers: TLayers);
...
{: This is a non visual control that is used to store and manage the list
  ...
  private
    ...
    fGridColor: TColor;
    fGridDeltaX,
    fGridDeltaY: TRealType;
    fUseSnap: boolean;
    fSnapX,
    fSnapY: TRealType;
    fUseOrtho,
    fEditMode,
    fPolarTracking: boolean;
    fPolarTrackingValue: TrealType;
    fSelectionMode: TSelectionMode; //(smSingle, smArea, smExtendet)
  ...
  public
    ...
    //added
    procedure BringToFront(const IDOrigin: LongInt);
  ...
  published
    ...
    property GridDeltaX: TRealType    read fGridDeltaX      write fGridDeltaX;
    property GridDeltaY: TRealType    read fGridDeltaY      write fGridDeltaY;
    property UseSnap:   boolean       read fUseSnap         write fUseSnap;
    property SnapX: TRealType         read fSnapX           write fSnapX;
    property SnapY: TRealType         read fSnapY           write fSnapY;
    property UseOrtho:  boolean       read fUseOrtho        write fUseOrtho;
    property EditMode:  boolean       read fEditMode        write fEditMode;
    property PolarTracking: boolean   read fPolarTracking   write fPolarTracking;
  ...
  private
    ...
    fShowControlPoints, fShowGrid, fShowGridMainAxes, fInUpdate: Boolean;
  ...
  public
    ...
    property  ShowGridMainAxes: boolean  read fShowGridMainAxes write fShowGridMainAxes;
  protected
  ...
  protected
    ...
    procedure SetAngle(AAngle: TRealType);  virtual;
  ...
  public
    ...
    function  GetPenSource: TPenSource; virtual; abstract;
    procedure SetPenSource(APenSource: TPenSource); virtual; abstract;
    function  GetPenColor: TColor;  virtual; abstract;
    procedure SetPenColor(APenColor: TColor); virtual; abstract;
    function  GetPenStyle: TPenStyle;  virtual; abstract;
    procedure SetPenStyle(APenStyle: TPenStyle); virtual; abstract;
    function  GetPenWidth: word;  virtual; abstract;
    procedure SetPenWidth(AValue: word); virtual; abstract;

    function  GetBrushSource: TBrushSource; virtual; abstract;
    procedure SetBrushSource(ABrushSource: TBrushSource); virtual; abstract;
    function  GetBrushColor: TColor;  virtual; abstract;
    procedure SetBrushColor(ABrushColor: TColor); virtual; abstract;
    function  GetBrushStyle: TBrushStyle;  virtual; abstract;
    procedure SetBrushStyle(ABrushStyle: TBrushStyle); virtual; abstract;
...
{ -----===== Starting Cs4CADPrgClass.pas =====----- }
  ...
  function BlockNameToString(ABlockName: TSourceBlockName): string;
...
//added
function  ExtendLine(startPoint, endPoint: TPoint2D; totalLength: TRealType): TPoint2D;    //  forward;
procedure RotatePointAroundCenter(APointSet: TPointsSet2D; AAngle: TRealType);
function  ObjectArea(APointsSet2D: TPointsSet2D): TRealType;
function  CADDirection(APointsSet: TPointsSet2D): TArcDirection;
function  OffsetPolygon2D(APointsSet2D: TPointsSet2D; AOffset: TRealType): TPointsSet2D;

const ISqrt2 = 0.707106781186547; // 1/sqrt(2)

var CADClipboard2D: TContainer2D;

    EditPenColor: TColor;
    EditPenStyle: TPenStyle;
    EditPenWidth: word;
    EditBrushColor: TColor;
    EditBrushStyle: TBrushStyle;

Implementation

uses CS4Shapes, Math, Dialogs, Forms;
...
begin
  ...
  N:=P0.X*P3.Y + P3.X*P1.Y + P1.X*P2.Y + P2.X*P0.Y - P3.X*P0.Y - P0.X*P2.Y - P2.X*P1.Y - P1.X*P3.Y;
  // Möglicherweise Abbruch wegen Divison durch Null
  If N=0 then Exit;
  // T berechnen (Matrize)
  T:=(P0.X*P1.Y + P1.X*P2.Y + P2.X*P0.Y - P1.X*P0.Y - P2.X*P1.Y - P0.X*P2.Y) / N;
  // Wenn T nicht im Intervall [0,1] liegt dann haben
  // beide Linien keinen Schnittpunkt
  If (T<0) or (T>1) then Exit;
  // S berechnen (Matrize)
  S:=(P0.X*P3.Y + P3.X*P2.Y + P2.X*P0.Y - P3.X*P0.Y - P2.X*P3.Y - P0.X*P2.Y) / N;
  // Wenn S nicht im Intervall [0,1] liegt dann haben
  // beide Linien keinen Schnittpunkt
  If (S<0) or (S>1) then Exit;
  // Berechnung mit T
  P4.X := P2.X+T*(P3.X-P2.X);
  P4.Y := P2.Y+T*(P3.Y-P2.Y);
  P4.W := 1.0;
  // Berechnung mit S
  //X:=Round(P0.X+S*(P1.X-P0.X));
  //Y:=Round(P0.Y+S*(P1.Y-P0.Y));
  result := true;
end;
...
procedure DrawArrows2D(const VT: TTransf2D; const Cnv: TDecorativeCanvas; const ClipRect2D: TRect2D; const DrawMode: Integer; P0, P1: TPoint2D; AArrowLength,  AArrowAngle: TRealType; AArrowStyle: word);
var a, b, c, BaseAngle, Angle1: TRealType;  TmpPoint2D: TPoint2D;
begin
  ...
  TmpPoint2D.X := P1.X - a;
  TmpPoint2D.Y := P1.Y - b;
  DrawLine2D(Cnv, P1, TmpPoint2D, ClipRect2D, VT);
...
var
  TmpVect: TVector2D;
  SnapAngle, Dist: TRealType;
begin
  if IsSamePoint2D(LastPt, Point2D(MaxCoord, MaxCoord)) then exit;
  TmpVect := Vector2D(LastPt, CurrSnappedPt);
  SnapAngle := DegToRad(Round(RadToDeg(ArcTan2(TmpVect.Y, TmpVect.X)) / AAngle) * AAngle);
  Dist := Round(PointDistance2D(LastPt, CurrSnappedPt));
  CurrSnappedPt.X := LastPt.X + Dist * Cos(SnapAngle);
  CurrSnappedPt.Y := LastPt.Y + Dist * Sin(SnapAngle);
end;

procedure Make45_2D(LastPt: TPoint2D; var CurrSnappedPt: TPoint2D);
begin
  MakePolarPoint2D(LastPt, CurrSnappedPt, 45.0);
end;
...
//added
function ExtendLine(startPoint, endPoint: TPoint2D; totalLength: TRealType): TPoint2D;
var
  dx, dy, currentLength, extensionLength: TRealType;
begin
  // Berechnen der Richtung der ursprünglichen Linie
  dx := endPoint.x - startPoint.x;
  dy := endPoint.y - startPoint.y;

  // Berechnen der aktuellen Länge der Linie
  currentLength := sqrt(dx * dx + dy * dy);

  // Berechnen der Länge, um die die Linie verlängert oder verkürzt werden muss
  extensionLength := totalLength - currentLength;

  // Berechnen des neuen Endpunkts
  result.x := endPoint.x + (dx * (extensionLength / currentLength));
  result.y := endPoint.y + (dy * (extensionLength / currentLength));
  result.w := endPoint.w;
end;

procedure RotatePointAroundCenter(APointSet: TPointsSet2D; AAngle: TRealType);
var i: Integer; dx, dy: TRealType; Center: TPoint2D;
begin
  Center.x := (APointSet.Extension.FirstEdge.x + APointSet.Extension.SecondEdge.x) / 2;
  Center.y := (APointSet.Extension.FirstEdge.y + APointSet.Extension.SecondEdge.y) / 2;

  for i := 0 to APointSet.Count - 1 do
  begin
    dx := APointSet[i].x - Center.x;
    dy := APointSet[i].y - Center.y;
    PVectPoints2D(APointSet.PointsReference)^[i].X := Center.x + (dx * cos(AAngle) - dy * sin(AAngle));
    PVectPoints2D(APointSet.PointsReference)^[i].Y := Center.y + (dx * sin(AAngle) + dy * cos(AAngle));
  end;
end;

function  ObjectArea(APointsSet2D: TPointsSet2D): TRealType;
var n, i: Integer; area: Real;
begin
  n := APointsSet2D.Count;
  result := 0;
  for i := 0 to n - 1 do
  begin
    result := result + APointsSet2D[i].x * APointsSet2D[(i + 1) mod n].y;
    result := result - APointsSet2D[(i + 1) mod n].x * APointsSet2D[i].y;
  end;
  result := Abs(result) / 2;
end;
...
var
  ...
  i, j, count: Integer;
  dx, dy, len, scaleFactor: Int64;
  nx, ny: Int64;
  const multiplicator = 10000;
begin
  count := APointsSet2D.Count;
  if count < 3 then // Need at least 3 points for a polygon
    Exit(nil);
  OffsetPoints := TPointsSet2D.Create(count);

  SetLength(OriginalPoints, count);

  // Store original points
  for i := 0 to count - 1 do
    OriginalPoints[i] := APointsSet2D.Points[i];

  // Calculate scale factor
  scaleFactor := Round(AOffset * multiplicator); // Multipliziere mit großem Wert, um Genauigkeit zu erhöhen

  // Offset points
  for i := 0 to count - 1 do
  begin
    j := (i + 1) mod count;
    dx := Round((OriginalPoints[j].X - OriginalPoints[i].X) * multiplicator); // Multipliziere mit großem Wert, um Genauigkeit zu erhöhen
    dy := Round((OriginalPoints[j].Y - OriginalPoints[i].Y) * multiplicator); // Multipliziere mit großem Wert, um Genauigkeit zu erhöhen
    len := Round(Sqrt(dx * dx + dy * dy));
    if len > 0 then
    begin
      nx := -dy;
      ny := dx;
      dx := (nx * scaleFactor) div len;
      dy := (ny * scaleFactor) div len;
      OffsetPoints.Points[i] := Point2D(( OriginalPoints[i].X + dx / multiplicator), (OriginalPoints[i].Y + dy / multiplicator)); // Teile durch den gleichen großen Wert, um den Ergebniswert in den normalen Bereich zu bringen
    end
    else
    begin
      OffsetPoints.Points[i] := Point2D((OriginalPoints[i].X), (OriginalPoints[i].Y));
    end;
  end;

  // Adjust points to ensure equal distance between old and new polygon
  {for i := 0 to count - 1 do
  begin
    j := (i + count - 1) mod count; // Previous point
    dx := Round((OffsetPoints.Points[j].X - OriginalPoints[j].X) * multiplicator); // Multipliziere mit großem Wert, um Genauigkeit zu erhöhen
    dy := Round((OffsetPoints.Points[j].Y - OriginalPoints[j].Y) * multiplicator); // Multipliziere mit großem Wert, um Genauigkeit zu erhöhen
    len := Round(Sqrt(dx * dx + dy * dy));
    if len > 0 then
    begin
      scaleFactor := Round(AOffset * multiplicator) div len; // Multipliziere mit großem Wert, um Genauigkeit zu erhöhen
      OffsetPoints.Points[i] := Point2D((OriginalPoints[i].X + dx * scaleFactor / multiplicator), (OriginalPoints[i].Y + dy * scaleFactor / multiplicator));
    end;
  end; }
...
function CrossProduct(p1, p2, p3: TPoint2D): TRealType;
begin
...
procedure TPointsSet2D.Reverse;
var
  i, j: Integer;
  Temp: TPoint2D;
begin
  if fCount <= 1 then Exit; // Kein Reverse notwendig für 0 oder 1 Punkt
  ...
  while i < j do
  begin
    // Punkte an den Positionen i und j tauschen
    Temp := Get(i);  // Temporärspeicher für Punkt an Position i
    Put(i, i, Get(j));  // Punkt an Position j wird an Position i gesetzt
    Put(j, j, Temp);    // Temporärer Punkt wird an Position j gesetzt
  ...
  // Events/Änderungen nach der Modifikation auslösen
  CallOnChange;
end;
...
///////////////////////////////////////////////////////
// TPosition2D/////////////////////////////////////////
constructor TPosition2DInsp.create(AOwner: TGraphicObject);
begin
  inherited create;
  fOwner := AOwner;
...
function  TPosition2DInsp.GetTop: TRealType;
begin
  if  fOwner <> nil then
    result := TObject2D(fOwner).Top
  else
    result := -99999999;
end;

procedure TPosition2DInsp.SetTop(ATop: TRealType);
begin
  if  fOwner <> nil then
    TObject2D(fOwner).Top := ATop;
end;

function  TPosition2DInsp.GetLeft: TRealType;
begin
  if  fOwner <> nil then
    result := TObject2D(fOwner).Left
  else
    result := -99999999;
end;

procedure TPosition2DInsp.SetLeft(ALeft: TRealType);
begin
  if  fOwner <> nil then
    TObject2D(fOwner).Left := ALeft;
end;

function  TPosition2DInsp.GetBottom: TRealType;
begin
  if  fOwner <> nil then
    result := TObject2D(fOwner).Bottom
  else
    result := -99999999;
end;

procedure TPosition2DInsp.SetBottom(ABottom: TRealType);
begin
  if  fOwner <> nil then
    TObject2D(fOwner).Bottom := ABottom;
end;

function  TPosition2DInsp.GetRight: TRealType;
begin
  if  fOwner <> nil then
    result := TObject2D(fOwner).Right
  else
    result := -99999999;
end;

procedure TPosition2DInsp.SetRight(ARight: TRealType);
begin
  if  fOwner <> nil then
   TObject2D(fOwner).Right := ARight;
end;

//TGrObjectInsp/////////////////////////////////////////////////////////////////

constructor TGrObjectInsp.create(AOwner: TGraphicObject);
begin
 inherited create;
 fOwner := AOwner;
end;

destructor  TGrObjectInsp.destroy;
begin
  inherited;
end;

function  TGrObjectInsp.GetID: LongInt;
begin
  result := fOwner.ID;
end;

function  TGrObjectInsp.GetClassName: string;
begin
  result := fOwner.ClassName;
end;

function  TGrObjectInsp.GetLayerName: TLayerName;
begin
  result := fOwner.LayerName;
end;

procedure TGrObjectInsp.SetLayerName(ALayerName: TLayerName);
begin
  fOwner.LayerName := ALayerName;
end;

function  TGrObjectInsp.GetLayerIDX: word;
begin
  result := fOwner.LayerIndex;
end;

procedure TGrObjectInsp.SetLayerIDX(AValue: word);
begin
  fOwner.LayerIndex := AValue;
end;

function  TGrObjectInsp.GetVisible: boolean;
begin
  result := fOwner.Visible;
end;

procedure TGrObjectInsp.SetVisible(AValue: boolean);
begin
  fOwner.Visible := AValue;
end;

function  TGrObjectInsp.GetEnabled: boolean;
begin
  result := fOwner.Enabled;
end;

procedure TGrObjectInsp.SetEnabled(AValue: boolean);
begin
  fOwner.Enabled := AValue;
end;

function  TGrObjectInsp.GetTag: LongInt;
begin
  result := fOwner.Tag;
end;

procedure TGrObjectInsp.SetTag(AValue: LongInt);
begin
  fOwner.Tag := AValue;
end;
...
destructor  TObject2DInsp.destroy;
begin
  fPosition2D.Free;
  inherited;
end;

function TObject2DInsp.GetAngle: TRealType;
begin
  result := TObject2D(fOwner).Angle;
end;
...
begin
  inherited Create;
  fGrObjectInsp := TGrObjectInsp.create(self);
  fID := ID;
  fLayerIndex := 0;
  fVisible := True;
  fEnabled := True;
...
begin
  ...
  with Stream do
   begin
     Read(fID, SizeOf(fID));
     Read(fLayerIndex, SizeOf(fLayerIndex));
     Read(fTag, SizeOf(fTag));

     Read(fReserveInt1, SizeOf(fReserveInt1));
     Read(fReserveInt2, SizeOf(fReserveInt2));
     Read(fReserveInt3, SizeOf(fReserveInt3));

     Read(fReserveStr1, SizeOf(fReserveStr1));
     Read(fReserveStr2, SizeOf(fReserveStr2));
     Read(fReserveStr3, SizeOf(fReserveStr3));
...
begin
  ...
  with Stream do
   begin
     ...
     Write(fLayerIndex, SizeOf(fLayerIndex));
     Write(fTag, SizeOf(fTag));

     Write(fReserveInt1, SizeOf(fReserveInt1));
     Write(fReserveInt2, SizeOf(fReserveInt2));
     Write(fReserveInt3, SizeOf(fReserveInt3));

     Write(fReserveStr1, SizeOf(fReserveStr1));
     Write(fReserveStr2, SizeOf(fReserveStr2));
     Write(fReserveStr3, SizeOf(fReserveStr3));
...
begin
  ...
  fReserveInt1 := Obj.fReserveInt1;
  fReserveInt2 := Obj.fReserveInt2;
  fReserveInt3 := Obj.fReserveInt3;

  fReserveStr1 := Obj.fReserveStr1;
  fReserveStr2 := Obj.fReserveStr2;
  fReserveStr3 := Obj.fReserveStr2;
end;
...
//added
function  TGraphicObject.GetLayerName: TLayerName;
var TmpLayer: TLayer;
begin
  if OwnerCAD = nil then exit;
  TmpLayer := OwnerCAD.Layers[fLayerIndex];
  if  TmpLayer <> nil then
    result := TmpLayer.Name
  else
    result := '0';
end;

function TGraphicObject.GetClassName: string;
begin
  result := self.ClassName;
end;

procedure TGraphicObject.SetLayerName(ALayerName: TLayerName);
var TmpLayer: TLayer;
begin
  if OwnerCAD = nil then exit;
  TmpLayer := OwnerCAD.Layers.LayerByName[ALayerName];
  if   TmpLayer <> nil then
    fLayerIndex := TmpLayer.LayerIndex
  else
    fLayerIndex := 0;
end;
...
procedure TLayer.Assign(ALayer: TLayer);
begin
  fPen.Assign(ALayer.Pen);
  fBrush.Assign(ALayer.Brush);
  fName   := ALayer.Name;
  fOpaque  := ALayer.Opaque;
  fVisible := ALayer.Visible;
  fActive  := ALayer.Active;
end;
...
begin
  inherited Create;
  fIdx := Idx;
  //fName := Format('Layer %d', [Idx]);  //original
  //replaced
  if fIdx = 0 then
    fName := '0'
  else
...
var
  ...
  TmpPenMode: TPenMode;
  TmpCosmetic: boolean;
  TmpFPPenEndCap: TFPPenEndCap;
  TmpJoinStyle : TFPPenJoinStyle;
begin
  if (not Assigned(APen)) or (not Assigned(AStream)) then exit;
  TmpColor := APen.Color;
  AStream.Write(TmpColor, SizeOf(TmpColor));
  TmpPenStyle := APen.Style;
  AStream.Write(TmpPenStyle, SizeOf(TmpPenStyle));
  TmpWidth := APen.Width;
  AStream.Write(TmpWidth, SizeOf(TmpWidth));
  {TmpPenMode := APen.Mode;
  AStream.Write(TmpPenMode, SizeOf(TmpPenMode));
  TmpCosmetic := APen.Cosmetic;
  AStream.Write(TmpCosmetic, SizeOf(TmpCosmetic));
  TmpFPPenEndCap := APen.EndCap;
  AStream.Write(TmpFPPenEndCap, SizeOf(TmpFPPenEndCap));
  TmpJoinStyle := APen.JoinStyle;
  AStream.Write(TmpJoinStyle, SizeOf(TmpJoinStyle));}
end;
...
begin
  if (not Assigned(APen)) or (not Assigned(AStream)) then exit;
  AStream.Read(TmpColor, SizeOf(TmpColor));
  APen.Color := TmpColor;
  AStream.Read(TmpPenStyle, SizeOf(TmpPenStyle));
  APen.Style := TmpPenStyle;
  AStream.Read(TmpWidth, SizeOf(TmpWidth));
  APen.Width := TmpWidth;
  {AStream.Read(TmpPenMode, SizeOf(TmpPenMode));
  APen.Mode := TmpPenMode;
  AStream.Read(TmpCosmetic, SizeOf(TmpCosmetic));
  APen.Cosmetic := TmpCosmetic;
  AStream.Read(TmpFPPenEndCap, SizeOf(TmpFPPenEndCap));
  APen.EndCap := TmpFPPenEndCap;
  AStream.Read(TmpJoinStyle, SizeOf(TmpJoinStyle));
  APen.JoinStyle := TmpJoinStyle;}
end;
...
begin
  ...
  for Cont := 0 to MAX_LAYER_NUMBER do
    if LowerCase(Nm) = LowerCase(TLayer(Flayers.Items[Cont]).fName) then
    begin
...
procedure TLayers.Assign(ALayers: TLayers);
var Cont: Byte;
begin
  for Cont := 0 to MAX_LAYER_NUMBER do
    self.Layers[cont].Assign(ALayers[cont]);
end;
...
begin
  ...
  for Cont := 0 to MAX_LAYER_NUMBER do
   begin
     ...
     //TmpLay.fName := Format('Layer %d', [Cont]); original
     //replaced
     if Cont = 0 then  TmpLay.fName := '0'
     else              TmpLay.fName := Format('Layer %d', [Cont]);
  ...
  while Stream.Position < Stream.Size do
   with Stream do
    begin
      ...
      if Cont = (MAX_LAYER_NUMBER + 1) then Break;
...
begin
  ...
  fUseOrtho           := false;
  fEditMode           := true;
  fPolarTracking      := false;
  fPolarTrackingValue := 45.0;
  fSelectionMode      := smArea;
end;
    ...
    repeat
      ...
      begin
        try
          TmpIter.DeleteCurrent;
          Inc(Deleted);
          TmpObj := TmpIter.Prev; // Nach dem Löschen rückwärts gehen
        except
          TmpObj := TmpIter.Prev; // Bei Fehler auch rückwärts gehen
        end;
      end;
      ...
      begin
        raise ECADSourceBlockIsReferenced.Create(
          'TCADCmp.FreeSourceBlocks: CADSys 4.0 Severe error'#10#13 +
          'The drawing contains circular reference of source blocks. They will not be deleted!');
        Exit;
      end;
...
begin
  with Stream do
   begin
     ...
     //added
     TmpWord := fCurrentLayer;
     Write(TmpWord, SizeOf(TmpWord));

     TmpColor := ColorToString(fBackgroundColor);
     Write(TmpColor, SizeOf(TmpColor));

     TmpBool := self.ShowGrid;
     Write(TmpBool, SizeOf(TmpBool));

     TmpColor := ColorToString(fGridColor);
     Write(TmpColor, SizeOf(TmpColor));
    ...
    TmpBool := fPolarTracking;
    Write(TmpBool, SizeOf(TmpBool));

    TmpReal := fPolarTrackingValue;
    Write(TmpReal, SizeOf(TmpReal));
...
begin
  ...
  //added
  Stream.Read(TmpWord, SizeOf(TmpWord));
  fCurrentLayer := TmpWord;

  Stream.Read(TmpColor, SizeOf(TmpColor));
  fBackgroundColor := StringToColor(TmpColor);

  Stream.Read(TmpBool, SizeOf(TmpBool));
  fShowGrid := TmpBool;

  Stream.Read(TmpColor, SizeOf(TmpColor));
  fGridColor := StringToColor(TmpColor);

  Stream.Read(TmpReal, SizeOf(TmpReal));
  fGridDeltaX :=  TmpReal;
  ...
  Stream.Read(TmpBool, SizeOf(TmpBool));
  fPolarTracking  :=  TmpBool;

  Stream.Read(TmpReal, SizeOf(TmpReal));
  fPolarTrackingValue := TmpReal;
...
procedure TCADCmp.SendToBack(const IDOrigin: LongInt);
var TmpIter: TExclusiveGraphicObjIterator;  TmpID: LongInt;
begin
  TmpIter := self.GetExclusiveListOfObjects;
  try
    TmpID := TmpIter.Last.ID;
  finally
    TmpIter.Free;
  end;
  if (IDOrigin <> TmpID) then
  begin
    MoveObject(IDOrigin, TmpID);
    MoveObject(TmpID, IDOrigin);
  end;
end;

procedure TCADCmp.BringToFront(const IDOrigin: LongInt);
var TmpIter: TExclusiveGraphicObjIterator;  TmpID: LongInt;
begin
  TmpIter := self.GetExclusiveListOfObjects;
  try
    TmpID := TmpIter.First.ID;
  finally
    TmpIter.Free;
  end;
  if (IDOrigin <> TmpID) then
...
procedure TCADCmp.BringForward(const IDOrigin: LongInt);
var TmpIter: TExclusiveGraphicObjIterator;  TmpID: LongInt;
begin
  TmpID := IDOrigin;
  TmpIter := self.GetExclusiveListOfObjects;
  try
    TmpIter.First;
    while (TmpIter.Current <> nil) and (TmpIter.Current.ID <> IDOrigin)  do
      TmpIter.Next;
    if (TmpIter.Current <> nil) then
      if (TmpIter.Current.ID = IDOrigin) then
      begin
         TmpIter.Prev;
         if (TmpIter.Current <> nil) then
           TmpID := TmpIter.Current.ID;
      end;
  finally
    TmpIter.Free;
  end;
...
procedure TCADCmp.SendBackwards(const IDOrigin: LongInt);
var TmpIter: TExclusiveGraphicObjIterator;  TmpID: LongInt;
begin
  TmpID := IDOrigin;
  TmpIter := self.GetExclusiveListOfObjects;
...
begin
  ...
  try
    ...
    begin
      if TmpIter.Current.LayerIndex = ALayerID then
      begin
        SetLength(IDArray, Length(IDArray) + 1);
        IDArray[Length(IDArray) - 1] := TmpIter.Current.ID;
      end;
      TmpIter.Next;
    end;
  finally
  ...
  for i := 0 to Length(IDArray) - 1   do
    ObjectList.Delete(IDArray[i]);
end;
...
//added
procedure TCADCmp.SetShowDirection(AValue: boolean);
begin
...
procedure DrawGridPoints(const Cnv: TCanvas; const D1, D2: TRealType);
var
  IX1, IX2, IY1, IY2: Integer;
  IX, IY: Integer;   P1, P2: TPoint2D;
begin
  IX1 := Ceil(ARect.Left / D1);
  IX2 := Floor(ARect.Right / D1);
  IY1 := Ceil(ARect.Bottom / D2);
  IY2 := Floor(ARect.Top / D2);
  for IX := IX1 to IX2 do
  begin
    for IY := IY1 to IY2 do
    begin
      P1 := ViewportToScreen(Point2D(IX * D1, IY * D2));
      Cnv.Pixels[Round(P1.X), Round(P1.Y)] := self.BackGroundColor xor self.GridColor;
    end;
  end;
end;
...
begin
  ...
  with Cnv do
   try
     ...
     if( (fGridSubX > 0) and (fGridSubY > 0) ) then
      begin
        TmpCnv.DecorativePen.SetPenStyle('10');
         DrawGridPoints(TmpCnv.Canvas, fGridDeltaX, fGridDeltaY);
     ...
     Pen.Style := psSolid;
     Pen.Color :=  self.BackGroundColor xor self.GridColor;
     //DrawGridLines(TmpCnv, fGridDeltaX, fGridDeltaY);
     // Draw the main axes.
     if fShowGridMainAxes then
     begin
       Pen.Width := 2;
...
begin
  StopRepaint;
  Temp := (FVisualWindow.Right - FVisualWindow.Left) / 20.0;   // original = 4.0
  TmpWin.Left := FVisualWindow.Left + Temp;
  TmpWin.Right := FVisualWindow.Right - Temp;
  TmpWin.W1 := 1.0;
  Temp := (FVisualWindow.Top - FVisualWindow.Bottom) / 20.0;
...
begin
  StopRepaint;
  Temp := (FVisualWindow.Right - FVisualWindow.Left) / 20.0;
  TmpWin.Left := FVisualWindow.Left - Temp;
  TmpWin.Right := FVisualWindow.Right + Temp;
  TmpWin.W1 := 1.0;
  Temp := (FVisualWindow.Top - FVisualWindow.Bottom) / 20.0;
...
function  TObject2D.GetAngle: TRealType;
begin
...
begin
  inherited Create(ID);
  fObject2DInsp := TObject2DInsp.create(self);
  fBox := Rect2D(0, 0, 0, 0);
  fModelTransform := nil;
  fSavedTransform := nil;
...
var TmpTransf: TTransf2D; TmpAngle: TRealType;
begin
  inherited;
  fObject2DInsp := TObject2DInsp.create(self);
  with Stream do
...
//Object Position
procedure TObject2D.SetTop(AValue:TRealType);
begin
  self.Transform(Translate2D(0, AValue - self.Box.Top));
end;

procedure TObject2D.SetLeft(AValue:TRealType);
begin
  self.Transform(Translate2D((AValue - self.Box.Left), 0));
end;

procedure TObject2D.SetBottom(AValue:TRealType);
begin
  self.Transform(Translate2D(0, AValue - self.Box.Bottom));
end;

procedure TObject2D.SetRight(AValue:TRealType);
begin
  self.Transform(Translate2D((AValue - self.Box.Right), 0));
end;

function TObject2D.GetRight: TRealType;
begin
  result := box.Right;
end;

function TObject2D.GetBottom: TRealType;
begin
  result := self.Box.Bottom;
end;

function TObject2D.GetLeft: TRealType;
begin
  result := self.Box.Left
 end;

function TObject2D.GetTop: TRealType;
begin
  result := box.Top;
end;

function TObject2D.GetLeftTop: TPoint2D;
begin
  result := Point2D(GetLeft, GetTop);
end;

function TObject2D.GetLeftBottom: TPoint2D;
begin
  result := Point2D(GetLeft, GetBottom);
end;

function TObject2D.GetRightBottom: TPoint2D;
begin
  result := Point2D(GetRight, GetBottom);
end;

function TObject2D.GetRightTop: TPoint2D;
begin
  result := Point2D(GetRight, GetTop);
end;

function TObject2D.GetLeftCenter: TPoint2D;
begin
  result := Point2D(GetLeft, (GetTop + GetBottom) / 2)
end;

function TObject2D.GetBottomCenter: TPoint2D;
begin
  result := Point2D((GetRight + GetLeft) /2 , GetBottom);
end;

function TObject2D.GetRightCenter: TPoint2D;
begin
  result := Point2D(GetRight, (GetBottom + GetTop) / 2);
end;

function TObject2D.GetTopCenter: TPoint2D;
begin
  result := Point2D((GetRight + GetLeft) /2 , GetTop);
end;

function TObject2D.GetMiddlePoint: TPoint2D;
begin
...
function TObject2D.GetMiddlePointX: TRealType;
begin
  result := StrToFloat(FormatFloat('0.0000000000', GetMiddlePoint.X));
  //result := GetMiddlePoint.X;
end;

function TObject2D.GetMiddlePointY: TRealType;
begin
  result := StrToFloat(FormatFloat('0.0000000000', GetMiddlePoint.Y))
  //result := GetMiddlePoint.Y;
end;

procedure TObject2D.SetMiddlePointX(AValue:TRealType);
var ToPt, DragPt: TPoint2D;
begin
  DragPt := self.GetMiddlePoint;
  ToPt   := Point2D(AValue, self.GetMiddlePointY);
  self.MoveTo(ToPt, DragPt);
end;

procedure TObject2D.SetMiddlePointY(AValue:TRealType);
var ToPt, DragPt: TPoint2D;
begin
  DragPt := self.GetMiddlePoint;
...
var TmpIter: TExclusiveGraphicObjIterator;
    TmpObj: TObject2D;
begin
  TmpIter := Objects.GetExclusiveIterator;
  TmpIter.First;
  try
    repeat
      if (TmpIter.Current is TLine2D) then
      begin
        TmpObj := TLine2D.Create(TmpIter.Current.ID, Point2D(0, 0), Point2D(0, 0));
        TmpObj.Assign(TLine2D(TmpIter.Current));
      end else

      if (TmpIter.Current is TSector2D) then
      begin
        TmpObj := TSector2D.Create(TmpIter.Current.ID, Point2D(0, 0), 0, 0, 0);
        TSector2D(TmpObj).Assign(TSector2D(TmpIter.Current));
      end else

      if (TmpIter.Current is TSegment2D) then
      begin
        TmpObj := TSegment2D.Create(TmpIter.Current.ID, Point2D(0, 0), 0, 0, 0);
        TSegment2D(TmpObj).Assign(TSegment2D(TmpIter.Current));
      end else

      if (TmpIter.Current is TEllipticalArc2D) then
      begin
        TmpObj := TEllipticalArc2D.Create(TmpIter.Current.ID, Point2D(0, 0), Point2D(0, 0), 0, 0);
        TEllipticalArc2D(TmpObj).Assign(TEllipticalArc2D(TmpIter.Current));
      end else

      if (TmpIter.Current is TPolygon2D) then
      begin
        TmpObj := TPolygon2D.Create(TmpIter.Current.ID, []);
        TPolygon2D(TmpObj).Assign(TPolygon2D(TmpIter.Current));
      end else

      if (TmpIter.Current is TPolyline2D) then
      begin
        TmpObj := TPolyline2D.Create(TmpIter.Current.ID, []);
        TPolyline2D(TmpObj).Assign(TPolyline2D(TmpIter.Current));
      end else

      if (TmpIter.Current is TFrame2D) then
      begin
        TmpObj := TFrame2D.Create(TmpIter.Current.ID, Point2D(0, 0), Point2D(0, 0));
        TFrame2D(TmpObj).Assign(TFrame2D(TmpIter.Current));
      end else

      if (TmpIter.Current is TBSpline2D) then
      begin
        TmpObj := TBSpline2D.Create(TmpIter.Current.ID, []);
        TBSpline2D(TmpObj).Assign(TBSpline2D(TmpIter.Current));
      end else

      if (TmpIter.Current is TCircularArc2D) then
      begin
        TmpObj := TCircularArc2D.Create(TmpIter.Current.ID, Point2D(0, 0), 0, 0, 0);
        TCircularArc2D(TmpObj).Assign(TCircularArc2D(TmpIter.Current));
      end else

      if (TmpIter.Current is TSymetricSymbol2D) then
      begin
        TmpObj := TSymetricSymbol2D.Create(TmpIter.Current.ID, Point2D(0, 0), 0);
        TSymetricSymbol2D(TmpObj).Assign(TSymetricSymbol2D(TmpIter.Current));
      end else

      if (TmpIter.Current is TASymetricSymbol2D) then
      begin
        TmpObj := TASymetricSymbol2D.Create(TmpIter.Current.ID, Point2D(0, 0), Point2D(0, 0));
        TASymetricSymbol2D(TmpObj).Assign(TASymetricSymbol2D(TmpIter.Current));
      end else
      ...
      if (TmpIter.Current is TEllipse2D) then
      begin
        TmpObj := TEllipse2D.Create(TmpIter.Current.ID, Point2D(0, 0), Point2D(0, 0));
        TEllipse2D(TmpObj).Assign(TEllipse2D(TmpIter.Current));
      end else

      if (TmpIter.Current is TText2D) then
      begin
        TmpObj := TText2D.Create(TmpIter.Current.ID, Rect2D(0, 0, 0, 0),   0, '');
        TText2D(TmpObj).Assign(TText2D(TmpIter.Current));
      end else

      if (TmpIter.Current is TJustifiedVectText2D) then
      begin
        TmpObj := TJustifiedVectText2D.Create(TmpIter.Current.ID, nil, Rect2D(0, 0, 0, 0), 0, '');
        TJustifiedVectText2D(TmpObj).Assign(TJustifiedVectText2D(TmpIter.Current));
      end else

      if (TmpIter.Current is TBitmap2D) then
      begin
        TmpObj := TBitmap2D.Create(TmpIter.Current.ID, Point2D(0, 0), Point2D(0, 0), nil);
        TBitmap2D(TmpObj).Assign(TBitmap2D(TmpIter.Current));
      end else

      if (TmpIter.Current is TBlock2D) then
      begin
        TmpObj := TBlock2D.Create(TmpIter.Current.ID, nil);
...
begin
  ...
  try
    ...
    Obj := TObject2D(TmpIter.First);

    while Obj <> nil do
    begin
      if (Obj is TPrimitive2D)then
      begin
        Cnv.Canvas.Pen.Color   := Obj.PenColor;
        Cnv.Canvas.Pen.Style   := Obj.PenStyle;
        Cnv.Canvas.Pen.Width   := Obj.PenWidth;
      end;

      if (Obj is TClosedPolyline2D) then
      begin
        Cnv.Canvas.Brush.Color  := TClosedPolyline2D(Obj).BrushColor;
        Cnv.Canvas.Brush.Style  := TClosedPolyline2D(Obj).BrushStyle;
      end;
      if (Obj is TClosedCurve2D) then
      begin
        Cnv.Canvas.Brush.Color  := TClosedCurve2D(Obj).BrushColor;
        Cnv.Canvas.Brush.Style  := TClosedCurve2D(Obj).BrushStyle;
      end;


       { Transform the object in the container. }
       Obj.Draw(TmpTransf, Cnv, ClipRect2D, DrawMode);
       Obj := TObject2D(TmpIter.Next);
    end;
    ...
    if (Obj is TClosedPolyline2D) or (Obj is TClosedCurve2D) then
    begin
...
begin
  ...
  fNReference := 0; //original
  //Stream.Read(fNReference, SizeOf(fNReference));
end;
...
begin
  ...
  //Stream.Write(fNReference, SizeOf(fNReference));
end;
...
var TmpIter: TExclusiveGraphicObjIterator;
    TmpObj: TObject2D;
begin
  TmpIter := self.SourceBlock.Objects.GetExclusiveIterator;
  TmpIter.First;
  try
    while TmpIter.Current <> nil do
    begin
      TObject2D(TmpIter.Current).Transform(ModelTransform);
      TmpIter.Next;
    end;
  finally
    TmpIter.Free;
...
begin
  ...
  with Stream do
   try
     ...
     while TmpObj <> nil do
      begin
        if Layers[TmpObj.LayerIndex].Streamable and TmpObj.fToBeSaved then
         begin
           TmpWord := CADSysFindClassIndex(TmpObj.ClassName);
           { Save the class index. }
           Write(TmpWord, SizeOf(TmpWord));
   ...
   try
     ...
     while TmpLong > 0 do
      begin
        ...
        CurrentLayer := TmpObj.LayerIndex;
...
begin
  ...
  try
    ...
    while Tmp <> nil do
     begin
       if fLayers[Tmp.LayerIndex].fVisible then
        begin
          try
...
begin
  ...
  with Obj as TObject2D do
   try
     if (CADCmp.Layers.SetCanvas(Cnv, LayerIndex)) then
      begin
...
begin
  ...
  with Obj as TObject2D do
   try
     ...
     //Draw(ViewportToScreenTransform, Cnv, ClipRect2D, DrawMode);
     Draw(ViewportToScreenTransform, Cnv, ClipRect2D, DRAWMODE_EDIT);
     if ShowControlPoints then
      begin
        Cnv.Canvas.Pen.Assign(RubberPen);
        Cnv.Canvas.Pen.Width := 1;
    ...
    while Tmp <> nil do
     begin
       if Tmp.IsVisible(CADRect, DrawMode) then
        begin
          fCADCmp2D.Layers.SetCanvas(TmpCanvas, Tmp.LayerIndex);
...
begin
  ...
  try
    ...
    while Tmp <> nil do
     with (CADCmp.Layers[Tmp.LayerIndex]) do
      begin
...
initialization
  EditPenColor            := clWhite;
  EditPenStyle            := psDot;
  EditPenWidth            := 1;

  EditBrushColor          := clRed;
  EditBrushStyle          := bsSolid;
