...
// -----===== Starting Cs4DXF2DConverter.pas =====-----
  ...
  protected
    ...
    procedure WriteCircle2D(Circle2D: TCircle2D);
    procedure WriteCircularArc2D(CircularArc2D: TCircularArc2D);
    procedure WriteEllipticalArc2D(EllipticalArc2D: TEllipticalArc2D);
    procedure WriteEllipse2D(Ellipse2D: TEllipse2D);
...
procedure TDXF2DExport.WriteCircle2D(Circle2D: TCircle2D);
var TmpPnt: TPoint2D;
begin
  with FDXFWrite, Circle2D do
   begin
     TmpPnt := TransformPoint2D(Points[0], ModelTransform);
     WriteGroup(10, TmpPnt.X);
     WriteGroup(20, TmpPnt.Y);
     WriteGroup(40, Circle2D.Radius);
   end;
end;

procedure TDXF2DExport.WriteCircularArc2D(CircularArc2D: TCircularArc2D);
var TmpPnt: TPoint2D;
begin
  with FDXFWrite, CircularArc2D do
   begin
     TmpPnt := TransformPoint2D(Points[0], ModelTransform);
     WriteGroup(10, TmpPnt.X);
     WriteGroup(20, TmpPnt.Y);
     WriteGroup(40, CircularArc2D.Radius);
     WriteGroup(50, RadToDeg(CircularArc2D.StartAngle));
     WriteGroup(51, RadToDeg(CircularArc2D.EndAngle));
   end;
end;

procedure TDXF2DExport.WriteEllipticalArc2D(EllipticalArc2D: TEllipticalArc2D);
var TmpPnt, P0, P1: TPoint2D; Center: TPoint2D; SA, EA, MajorAxis, MinorAxis: TRealType;
begin
  with FDXFWrite, EllipticalArc2D do
  begin
    P0 := EllipticalArc2D.Points[0];
    P1 := EllipticalArc2D.Points[1];
    P0 := TransformPoint2D(P0, EllipticalArc2D.ModelTransform);
    P1 := TransformPoint2D(P1, EllipticalArc2D.ModelTransform);

    SA := EllipticalArc2D.StartAngle;
    EA := EllipticalArc2D.EndAngle;
    SA := RadToDeg(SA);
    EA := RadToDeg(EA);
    ...
    TmpPnt := TransformPoint2D(Center, EllipticalArc2D.ModelTransform);
...
procedure TDXF2DExport.WriteEllipse2D(Ellipse2D: TEllipse2D);
var TmpPnt: TPoint2D; Center, P0, P1: TPoint2D; MajorAxis, MinorAxis: Double;
begin
  with FDXFWrite, Ellipse2D do
  begin
    P0 := Ellipse2D.Points[0];
    P0 := TransformPoint2D(P0, Ellipse2D.ModelTransform);
    P1 := Ellipse2D.Points[1];
    P1 := TransformPoint2D(P1, Ellipse2D.ModelTransform);
...
begin
  with FDXFWrite, Frm do
   begin
     ...
     WriteGroup(0, 'VERTEX');
     WriteGroup(8, FCADCmp2D.Layers[Frm.LayerIndex].Name);
     TmpPnt := TransformPoint2D(Points[0], ModelTransform);
     WriteGroup(10, TmpPnt.X);
     WriteGroup(20, TmpPnt.Y);
     WriteGroup(30, 0);
   ...
   begin
     ...
     try
       ...
       for Count := 0 to ProfilePoints.Count - 1 do
        begin
          WriteGroup(0, 'VERTEX');
          WriteGroup(8, FCADCmp2D.Layers[Curve.LayerIndex].Name);
  ...
  with FDXFWrite, Poly do
   begin
     ...
     for Count := 0 to Points.Count - 1 do
      begin
        WriteGroup(0, 'VERTEX');
        WriteGroup(8, FCADCmp2D.Layers[Poly.LayerIndex].Name);
...
begin
  with FDXFWrite do
   if Obj is TLine2D then
    begin
      WriteGroup(0, 'LINE');
      WriteGroup(8, FCADCmp2D.Layers[Obj.LayerIndex].Name);
   ...
   begin
     WriteGroup(0, 'ARC');
     WriteGroup(8, FCADCmp2D.Layers[Obj.LayerIndex].Name);
     WriteCircularArc2D(Obj as TCircularArc2D);
   end
   ...
   begin
     ...
     WriteGroup(8, FCADCmp2D.Layers[Obj.LayerIndex].Name);
     WriteEllipticalArc2D(Obj as TEllipticalArc2D);
   end
