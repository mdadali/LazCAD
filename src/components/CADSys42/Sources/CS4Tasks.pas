   ...
   with TCAD2DSelectObjectsParam(Param), TCADCmp2D(CADPrg.Viewport.CADCmp) do
    begin
      ...
      if not InputQuery('Define block', 'Name', TmpStr) then
       ...
       TmpBlk := TCADPrg2D(CADPrg).Viewport2D.CADCmp2D.FindSourceBlock(TmpStr);
       if  (TmpBlk <> nil) then
         TmpStr2 := TmpBlk.Name;
         if LowerCase(StringToBlockName(TmpStr2)) = LowerCase(StringToBlockName(TmpStr)) then
         begin
           MessageDlg('Sorceblock ' + TmpStr + ' exists!' , mtError, [mbCancel], 0);
           NextState := CADPrg.DefaultState;
           Param.Free;
           Param := nil;
           exit;
...
begin
  ...
  begin
    if AOutLine2D.Direction = Clockwise then
      AOutLine2D.Direction := CounterClockwise
    else
      AOutLine2D.Direction := Clockwise;
    exit;
  end;
...
begin
  if Ellipse2D.Direction  = Clockwise then
    Ellipse2D.Direction  := CounterClockwise
  else
    Ellipse2D.Direction  := Clockwise;
end;

procedure  ReverseCircle2D(Circle2D: TCircle2D);
begin
  if Circle2D.Direction  = Clockwise then
    Circle2D.Direction  := CounterClockwise
  else
    Circle2D.Direction  := Clockwise;
end;

procedure ReverseFrame2D(var AFrame2D: TFrame2D);
begin
  if AFrame2D.Direction  = CounterClockwise then
    AFrame2D.Direction := Clockwise
  else
    AFrame2D.Direction := CounterClockwise;
end;
