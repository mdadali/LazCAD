    ...
    begin
      ...
      try
        ...
        while TmpIter.Current <> nil do
         begin
           TCADViewport2D(Tmpiter.Current.OwnerCAD.Viewports[0]).DrawObject2DWithRubber(TObject2D(Tmpiter.Current), true);
  ...
  with LocalPrg do
   begin
     ...
     StartOperation(TCAD2DDrawArcPrimitive, TCAD2DDrawArcPrimitiveParam.Create(nil,
        TEllipticalArc2D.Create(-1, Point2D(0, 0), Point2D(0, 0), 0, 0)));
   end;
end;
...
begin
  ...
  TmpText := TText2D.Create(0, Rect2D(1, 70, 99, 99), 20, 'Hello world');
  TmpText.DrawBox := False;
  TmpText.ClippingFlags := DT_CENTER;
  LocalCAD.AddObject(-1, TmpText);
end;
...
procedure TForm1.LocalViewMouseMove2D(Sender: TObject; Shift: TShiftState; WX, WY: Single; X, Y: Integer);
...
{  if SSLeft in Shift then
  begin
    ...
    begin
      TmpObj := LocalView.PickObject(LocalPrg.CurrentViewportSnappedPoint, 20, False, TmpN);
      //if  (TmpN = PICK_ONOBJECT) or (TmpN = PICK_INBBOX) then
      if  (TmpN = PICK_ONOBJECT)  then
      begin
        TmpObj.ApplyTransform;
        LocalView.Cursor := crDrag;
        MoveObject(TmpObj);
        LocalView.Repaint;
        LocalView.DrawObject2DWithRubber(TmpObj, true);
      end;
      if  (TmpN >=0) then
      begin
        TmpObj.ApplyTransform;
        EditObject(TmpObj, TmpN);
        LocalView.Repaint;
        LocalView.DrawObject2DWithRubber(TmpObj, true);
      end;
    end;
...
procedure TForm1.EditObject(AObject: TObject2D; APointNr: integer);
var TmpPrimitive2D: TPrimitive2D;  hTrans:TTransf2D;
begin
  TmpPrimitive2D := TPrimitive2D(AObject);
  TmpPrimitive2D.Points.DisableEvents := true;
  //PVectPoints2D(TmpPrimitive2D.Points.PointsReference)^[APointNr]:= TransformPoint2D(CADPrg2D1.CurrentViewportPoint, TmpPrimitive2D.ModelTransform);

  hTrans := TmpPrimitive2D.ModelTransform;

  TmpPrimitive2D.Points[APointNr] := TransformPoint2D(LocalPrg.CurrentViewportSnappedPoint, hTrans); //TransformPoint2D(CADPrg2D1.CurrentViewportSnappedPoint, AObject.ModelTransform);

  TmpPrimitive2D.UpdateExtension(nil);
  TmpPrimitive2D.Points.DisableEvents := false;
end;

procedure TForm1.MoveObject(AObject: TObject2D);
var i: Integer; diffX, diffY: TRealType; TmpPrimitive2D: TPrimitive2D;
     PointsCount: word;   TmpPoint2D: TPoint2D;
begin
  TmpPrimitive2D := TPrimitive2D(AObject);
  TPrimitive2D(AObject).Points.DisableEvents := true;

  PointsCount :=  TmpPrimitive2D.Points.Count;

  diffX := LocalPrg.CurrentViewportSnappedPoint.X - MoveBasePoint.X;
  diffY := LocalPrg.CurrentViewportSnappedPoint.Y - MoveBasePoint.Y;

 // if TmpPrimitive2D.HasTransform then
    //TmpPrimitive2D.Transform(Translate2D(diffX, diffY) ) ;
  //else
    //TmpPrimitive2D.Transform(Translate2D(diffX, diffY) )
  {for i := 0 to  TmpPrimitive2D.Points.Count - 1 do
  begin
    PVectPoints2D(TmpPrimitive2D.Points.PointsReference)^[i].X :=
        PVectPoints2D(TmpPrimitive2D.Points.PointsReference)^[i].X + diffX;
    PVectPoints2D(TmpPrimitive2D.Points.PointsReference)^[i].Y :=
        PVectPoints2D(TmpPrimitive2D.Points.PointsReference)^[i].Y + diffY;
  end;
  }
  MoveBasePoint := LocalPrg.CurrentViewportSnappedPoint;
  TmpPrimitive2D.UpdateExtension(nil);
  TPrimitive2D(AObject).Points.DisableEvents := false;
end;
