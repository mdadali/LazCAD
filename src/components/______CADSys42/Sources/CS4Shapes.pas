...
type
  ...
  public
    ...
    property  PenSource: TPenSource read fPenSource write fPenSource;
  published
  ...
  private
    ...
    function  GetAngle: TRealType;         override;
...
function   TPrim2DInsp.GetPen: TPen;
begin
  result :=  TPrimitive2D(fOwner).Pen;
end;

procedure  TPrim2DInsp.SetPen(AValue: TPen);
begin
  TPrimitive2D(fOwner).Pen := AValue;
end;

function   TPrim2DInsp.GetPenSource: TPenSource;
begin
  result := TPrimitive2D(fOwner).PenSource;
end;

procedure  TPrim2DInsp.SetPenSource(APenSource: TPenSource);
begin
...
function   TSimplePrim2DInsp.GetAngle: TRealType;
begin
  result := TSimplePrimitive2D(fOwner).Angle;
end;

procedure  TSimplePrim2DInsp.SetAngle(AValue: TRealType);
begin
  TSimplePrimitive2D(fOwner).Angle := AValue;
end;
...
function  TClosedPolyline2DInsp.GetBrushSource: TBrushSource;
begin
  result := TClosedPolyline2D(fOwner).BrushSource;
end;

procedure TClosedPolyline2DInsp.SetBrushSource(ABrushSource: TBrushSource);
begin
  TClosedPolyline2D(fOwner).BrushSource := ABrushSource;
end;

function  TClosedPolyline2DInsp.GetBrush: TBrush;
begin
  result := TClosedPolyline2D(fOwner).Brush;
end;

procedure TClosedPolyline2DInsp.SetBrush(ABrush: TBrush);
begin
  TClosedPolyline2D(fOwner).Brush := ABrush;
end;
...
function  TClosedCurve2DInsp.GetBrush: TBrush;
begin
  result := TClosedCurve2D(fOwner).Brush;
end;

procedure TClosedCurve2DInsp.SetBrush(ABrush: TBrush);
begin
  TClosedCurve2D(fOwner).Brush := ABrush;
end;

function  TClosedCurve2DInsp.GetBrushSource: TBrushSource;
begin
  result := TClosedCurve2D(fOwner).BrushSource;
end;

procedure TClosedCurve2DInsp.SetBrushSource(ABrushSource: TBrushSource);
begin
...
begin
  ...
  begin // Per default non aggiunge i punti.
    ...
    fAngle         := (Obj as TSimplePrimitive2D).Angle;
    fShowDirection := (Obj as TSimplePrimitive2D).fShowDirection;
    fPoints.Clear;
  end;
end;
...
function  TSimplePrimitive2D.GetAngle: TRealType;
begin
  //CADSys4.AngleFromPoints2D(self.);
  result := RadToDeg(fAngle);
end;

procedure TSimplePrimitive2D.SetAngle(AAngle: TRealType);
var hPoints: TPointsSet2D;
begin
  fAngle := DegToRad(AAngle);
  self.Transform(Rotate2D(fAngle));
...
procedure TDirectionalCurve2D.Assign(const Obj: TGraphicObject);
begin
  if (Obj = Self) then
    Exit;
  inherited Assign(Obj);
  if (Obj is TDirectionalCurve2D) then
    if Assigned(fBrush) then
    begin
      fDirection   :=  (Obj as TClosedCurve2D).Direction;
    end;
end;
...
begin
  ...
  self.Brush.Color := TmpBrushColor;
  Stream.Read(TmpBrushStyle, SizeOf(TmpBrushStyle));
  self.Brush.Style := TmpBrushStyle;
end;
...
begin
  ...
  TmpBrushColor := self.fBrush.Color;
  Stream.Write(TmpBrushColor, SizeOf(TmpBrushColor));
  TmpBrushStyle := self.fBrush.Style;
...
begin
  ...
  if (Obj is TClosedCurve2D) then
    ...
    begin
      ...
      fBrush.Color :=  (Obj as TClosedCurve2D).Brush.Color;
      fBrush.Style :=  (Obj as TClosedCurve2D).Brush.Style;
    end;
end;
...
begin
  case self.fBrushSource of
    ...
    bsCustom: begin
      ...
      Cnv.Canvas.Brush.Color := self.Brush.Color;
      Cnv.Canvas.Brush.Style := self.Brush.Style;
    end;
    else begin
      //
    end;
        ...
        //TmpPen := TPen.Create;
         ...
         case  DrawMode of
           ...
           DRAWMODE_NORMAL: begin
             Cnv.Canvas.Pen.Color     := self.Pen.Color;
             Cnv.Canvas.Pen.Style     := self.Pen.Style;
             Cnv.Canvas.Pen.Width     := self.Pen.Width;
...
begin
  ...
  Stream.Read(TmpBrushSource, SizeOf(TmpBrushSource));
  self.BrushSource := TmpBrushSource;
  if Assigned(fBrush) then
  begin
    Stream.Read(TmpBrushColor, SizeOf(TmpBrushColor));
    self.fBrush.Color := TmpBrushColor;
    Stream.Read(TmpBrushStyle, SizeOf(TmpBrushStyle));
    self.fBrush.Style := TmpBrushStyle;
  end;
end;
...
begin
  ...
  TmpBrushSource :=  self.BrushSource;
  Stream.Write(TmpBrushSource, SizeOf(TmpBrushSource));
  if Assigned(fBrush) then
  begin
    TmpBrushColor := self.fBrush.Color;
...
begin
  ...
  if (Obj is TClosedPolyline2D) then
    if Assigned(fBrush) then
    begin
      fDirection   :=  (Obj as TClosedPolyline2D).Direction;
      fBrushSource :=  (Obj as TClosedPolyline2D).BrushSource;
      fBrush.Color :=  (Obj as TClosedPolyline2D).Brush.Color;
      fBrush.Style :=  (Obj as TClosedPolyline2D).Brush.Style;
    end;
end;
...
begin
  ...
  begin // Per default non aggiunge i punti.
    ...
    begin
      ...
      fPen.Color :=  (Obj as TPrimitive2D).fPen.Color;
      fPen.Style :=  (Obj as TPrimitive2D).fPen.Style;
      fPen.Width :=  (Obj as TPrimitive2D).fPen.Width;
    end;
    fPoints.Clear;
  end;
end;
...
begin
  ...
  // Fügen Sie Punkte zum Bogen hinzu
  for Cont := 0 to NPts - 1 do
  begin
  ...
  // Optional: Schließen Sie den Bogen
  // PP.Add(PP[0]);
end; }
...
begin
  ...
  begin
    ...
    fRadius        := (Obj as TCircularArc2D).Radius;
...
begin
  ...
  if (Obj is TCircle2D) then
   begin
     Points.Copy(TPrimitive2D(Obj).Points, 0, 1);
     Points.GrowingEnabled := False;
     fRadius      := (Obj as TCircle2D).Radius;
     fStartAngle  := (Obj as TCircle2D).StartAngle;
     self.SetCenterPointX((Obj as TCircle2D).GetCenterPointX);
     self.SetCenterPointY((Obj as TCircle2D).GetCenterPointY);
   end;
end;
   ...
   begin
     if (Obj is TFrame2D) then   //added
      begin
        ...
        fWidth       := (Obj as TFrame2D).Width;
        fHeight      := (Obj as TFrame2D).Height;
      end;
     Points.Copy(TPrimitive2D(Obj).Points, 0, 1);
     Points.GrowingEnabled := False;
   end;
...
begin
  ...
  if (Obj is TEllipse2D) or (Obj is TFrame2D) or (Obj is TEllipticalArc2D) then
   begin
     fWidth  := (Obj as TEllipse2D).Width;
     fHeight := (Obj as TEllipse2D).Height;
     Points.Copy(TPrimitive2D(Obj).Points, 0, 1);
     Points.GrowingEnabled := False;
   end;
end;
